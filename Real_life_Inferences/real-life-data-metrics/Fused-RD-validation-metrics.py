...
OUT_DIR_SIM   = Path.cwd() / "outputs_full_fused"
...
OUT_DIR_REAL  = Path.cwd() / "outputs_full_real-life_fused"
...
def evaluate_dataset(
    ...
    for tag, pred in [("WITH-W", eval_df["pred_env"]), ("NO-W", eval_df["pred_base"])]:
        ...
        # Save confusion matrix and the report
    ...
    print(f"(delta) Brier   mean (NO-W − WITH-W): {d_br.mean():.4f}   p = {p_br:.3g}")
    ...
    #Plots (ROC, Histogram of probability shift)
    if _can_plot_roc(y):
...
# Combined global metric table (simulation vs real-life)
