"""
Code structure:
- Train Bayesian networks on simulated data (with & without environment width and only the gaze variable)
- Evaluate hard & probabilistic metrics on simulation test
...
OUT_DIR_SIM   = Path.cwd() / "outputs_full_gaze"
...
OUT_DIR_REAL  = Path.cwd() / "outputs_full_real-life_gaze"
...
WIDTH_CUT = 2.0       
GAZE_SCORE_CUT = 0.80 # this thereshold is based on the Looking model that is cited in the literature review
...
df_sim["G"] = (df_sim["gaze_angle"] <= GAZE_CUT).astype(int) 
...
# Fit two networks (ON SIMULATION TRAIN)
...
def evaluate_dataset_hard_only(
    ...
    for w_val in [0, 1]:
        ...
        for tag, pred_col in [("WITH-W", "pred_env"), ("NO-W", "pred_base")]:
            ...
            # Save per-width confusion matrix and the report
...
df_real["G"] = (df_real["gaze_score"] >= GAZE_SCORE_CUT).astype(int)  
...
# Combined global metric table (SIM vs REAL)
