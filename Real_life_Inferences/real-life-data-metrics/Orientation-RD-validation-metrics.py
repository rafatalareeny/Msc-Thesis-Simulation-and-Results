"""
Code structure:
- Train Bayesian networks on simulated data (with & without environment width and only the body orientation variable)
- Evaluate hard & probabilistic metrics on simulatin test
- Validate on real-life dataset using identical evaluation
- Outputs include CPDs, hard metrics (overall and per width W), probabilistic metrics (overall),
  ROC curves, probability-shift histograms, and a global comparison CSV.
"""
...
# Simulated dataset
"""Train test splits"""
CSV_PATH_SIM  = Path("Simulation_dataset.csv")
OUT_DIR_SIM   = Path.cwd() / "outputs_full_Orientation"
OUT_DIR_SIM.mkdir(parents=True, exist_ok=True)

# Real-life dataset
"""used only for evaluatoin with no refiting"""
CSV_PATH_REAL = Path("real-life-dataset-logs.csv")
OUT_DIR_REAL  = Path.cwd() / "outputs_full_real-life_Orientation"
OUT_DIR_REAL.mkdir(parents=True, exist_ok=True)

SEED      = 42
# Simulation discretization thresholds
"""Thereshold values used for the nimarization of the data. these values are unified for both the simulation and evaluation (real-life) dataset"""
ANGLE_CUT = 20.0      # deg, for sim: body_angle <= 20 => facing (B=1)

# Real-life discretization thresholds
WIDTH_CUT = 2.0       
B_MIN, B_MAX = 160.0, 200.0   

# Load & preprocess the simulation data (make the dataframe and the discretization based on the thresholds set above)
""""""
df_sim = pd.read_csv(CSV_PATH_SIM)
# Discretize 
df_sim["W"] = (df_sim["width"] < WIDTH_CUT).astype(int)      
df_sim["B"] = (df_sim["body_angle"] <= ANGLE_CUT).astype(int)
df_sim["I"] = df_sim["ground_truth"].astype(int)            
...
"""Construct the BN of the environment and with the environment networks to evluate the influence of the environment on the probability distribution"""
...
edges_env  = [("W","B"), ("W","I"), ("B","I")]
model_env, infer_env = fit_bn(edges_env, ["W","B","I"], "WITH-W")

# Without environment width
edges_base = [("B","I")]
model_base, infer_base = fit_bn(edges_base, ["B","I"], "NO-W")

#4) evalutation for the datasets 
...
def evaluate_dataset_hard_only(
    ...
    cols_to_keep = [c for c in ["width","W","B","I","pred_env","pred_base"] if c in eval_df.columns]
    eval_df[cols_to_keep].to_csv(out_dir / "predictions.csv", index=False)

    # Hard metrics for overall width cases
    ...
    # Hard metrics (for each width case (per width))
    ...
    for w_val in [0, 1]:
        ...
        for tag, pred_col in [("WITH-W", "pred_env"), ("NO-W", "pred_base")]:
            ...
            # Save per-width confusion matrix 
...
# Evaluate on simulation test dataset using the same function 
...
# Real-life validation (load, discretize, evaluate)
...
df_real["W"] = (df_real["width"] < WIDTH_CUT).astype(int)
...
# Combined global metric table (simulation vs real-lifedataset)

metrics_both = pd.concat([metrics_sim, metrics_real], ignore_index=True)
metrics_both.to_csv(Path.cwd() / "metrics_global__sim_vs_real_Bonly.csv", index=False)
print("\n=== Global hard-metric comparison saved to metrics_global__sim_vs_real_Bonly.csv ===")
