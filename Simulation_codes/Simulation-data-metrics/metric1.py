...
# 1) Load & binarize your trial_features.csv exactly as before:
df = pd.read_csv("trial_features4.csv")
df['W'] = (df.width < 2.0).astype(int)
df['B'] = (df.body_angle <= 20.0).astype(int)
df['G'] = (df.gaze_angle <= 20.0).astype(int)
df['I'] = df.ground_truth

# 2) Split into train/test (here we’ll do a simple 80/20 stratified split)
from sklearn.model_selection import train_test_split
train, test = train_test_split(df, test_size=0.2, stratify=df.I, random_state=42)

# 3) Define & fit your network (same structure every time)
...
# 4) Prepare an inference engine
infer = VariableElimination(model)

# 5) Run inference on each test instance
preds = []
for _, row in test.iterrows():
    evidence = {'W': int(row.W),
                'B': int(row.B),
                'G': int(row.G)}
    q = infer.query(['I'], evidence=evidence)
    # q.values is [P(I=0), P(I=1)]
...
• Binarises:
      ...
      B : body  angle     ≤ 20 °   → 1
      G : gaze  angle     ≤ 20 °   → 1
• Trains network   W → {B,G,I},  B → I,  G → I
...
OUT_DIR  = Path(os.getcwd()) / "outputs"   # folder for confusion-matrix files
...
# Bin-encode the three predictors
df["W"] = (df.width      < 2.0 ).astype(int)   # env.: narrow vs wide
...
# ------------------------------------------------------------------
model = DiscreteBayesianNetwork(
    [("W", "B"), ("W", "G"), ("W", "I"),   # environment influences B, G, I
     ("B", "I"), ("G", "I")]               # body & gaze influence intent
)
...
probs = []
for _, row in test.iterrows():
    evidence = {"W": int(row.W), "B": int(row.B), "G": int(row.G)}  # adapt per model
    q = infer.query(["I"], evidence=evidence)
