"""
Bayesian-network intent classifier (B → I only) + evaluation utilities
---------------------------------------------------------------------
• Loads trial_features4.csv
• Binarises:
      B : body  angle ≤ 20 ° → 1, else 0
• Trains network   B → I
...
OUT_DIR  = Path(os.getcwd()) / "outputs_monoB"  # separate folder
...
df["B"] = (df.body_angle <= 20.0).astype(int)   # sole predictor
...
# ------------------------------------------------------------------
model = DiscreteBayesianNetwork([("B", "I")])   # 1-edge BN
