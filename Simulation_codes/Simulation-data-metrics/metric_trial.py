...
probabilistic evaluation, significance tests and visualisations.
"""
...
# ------------------------------------------------------------------
'''
CSV_PATH  = Path("trial_features6.csv")
OUT_DIR   = Path.cwd() / "outputs_full"
...
df["B"] = (df.body_angle <= 20).astype(int)   # 1 = facing
df["G"] = (df.gaze_angle <= 15).astype(int)   # 1 = looking
...
# df["B"] = ((df.body_angle >= 160) & (df.body_angle < 200)).astype(int)
# df["G"] = (df.gaze_score >= 0.8).astype(int)   # 1 = looking
# df["I"] = df.ground_truth                            # 0 avoid / 1 obstruct
...
# ------------------------------------------------------------------
# 9) “Flip-W” table (how much P changes when W goes 0→1)
# ------------------------------------------------------------------
rows = []
for b, g in itertools.product([0, 1], [0, 1]):
    p_wide   = infer_env.query(["I"], evidence={"W": 0, "B": b, "G": g}).values[1]
    p_narrow = infer_env.query(["I"], evidence={"W": 1, "B": b, "G": g}).values[1]
    rows.append({"B": b, "G": g, "deltaP": p_narrow - p_wide})

flip_tbl = pd.DataFrame(rows).pivot(index="B", columns="G", values="deltaP")
flip_tbl.to_csv(OUT_DIR/"deltaP_flipW.csv")
print("\n=== ΔP(I=1) when corridor goes wide→narrow ===")
print(flip_tbl.to_string(float_format="{:+.3f}".format))
...
# Reliability diagram (overlay both models)
frac_e, mean_e = calibration_curve(test.I, test.prob_env,  n_bins=10)
frac_b, mean_b = calibration_curve(test.I, test.prob_base, n_bins=10)
plt.figure()
plt.plot(mean_e, frac_e, marker="o", label="WITH-W")
plt.plot(mean_b, frac_b, marker="s", label="NO-W")
plt.plot([0,1], [0,1], linestyle=":")
plt.xlabel("Mean predicted probability"); plt.ylabel("Empirical frequency")
plt.title("Reliability diagram"); plt.legend()
plt.savefig(OUT_DIR/"reliability_overlay.png", dpi=300); plt.close()

# Histogram of probability differences
plt.figure()
plt.hist(test.prob_env - test.prob_base, bins=30)
plt.xlabel("P_withW − P_noW"); plt.ylabel("Count")
plt.title("Shift in predicted probability due to environment")
