...
OUT_DIR   = Path.cwd() / "outputs_orientation_real-life"
...
test_out = test[["width","W","G","I","prob_env","prob_base","pred_env","pred_base"]]
...
print("\n===== HARD METRICS BY WIDTH =====")
for w_val, g in test.groupby("width"):
    y_true = g["I"].values

    # WITH-W model
    y_pred_env = g["pred_env"].values
    print(f"\n--- width = {w_val:.2f} m | WITH-W ---")
    print(classification_report(y_true, y_pred_env, target_names=["avoid", "obstruct"]))
    print("Accuracy (WITH-W):", accuracy_score(y_true, y_pred_env))

    # save confusion matrix image & CSV for this width (WITH-W)
    w_tag = f"{w_val:.2f}m".replace(".", "p")
    save_confmat(y_true, y_pred_env, ["avoid", "obstruct"], stem=f"cm_WITHW_width_{w_tag}")

    # NO-W model
    y_pred_base = g["pred_base"].values
    print(f"\n--- width = {w_val:.2f} m | NO-W ---")
    print(classification_report(y_true, y_pred_base, target_names=["avoid", "obstruct"]))
    print("Accuracy (NO-W):", accuracy_score(y_true, y_pred_base))

    # save confusion matrix image & CSV for this width (NO-W)
    save_confmat(y_true, y_pred_base, ["avoid", "obstruct"], stem=f"cm_NOW_width_{w_tag}")
# ------------------------------------------------------------------
