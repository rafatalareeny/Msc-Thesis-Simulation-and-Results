#!/usr/bin/env python3
"""
End-to-end pipeline (body orientation only, hard metrics only):
- Train Bayesian networks on simulated data (WITH-W vs NO-W; variables: W, B, I)
...
- Outputs:
  ...
  * Global hard-metrics comparison CSV (accuracy & F1-macro): metrics_global__sim_vs_real_Bonly.csv
"""
...
OUT_DIR_SIM   = Path.cwd() / "outputs_full_Bonly"
...
OUT_DIR_REAL  = Path.cwd() / "outputs_full_real-life_Bonly"
OUT_DIR_REAL.mkdir(parents=True, exist_ok=True)

SEED      = 42
# Simulation discretization thresholds for body
...
B_MIN, B_MAX = 160.0, 200.0   # deg, for real-life: B=1 when in [160, 200]

# ------------------------------------------------------------------
# 2) Load & preprocess (SIMULATION)
# ------------------------------------------------------------------
df_sim = pd.read_csv(CSV_PATH_SIM)
# Discretize (NO gaze used)
...
def evaluate_dataset_hard_only(
    ...
    Saves:
      - predictions.csv (W,B,I,pred_env,pred_base)
...
# ------------------------------------------------------------------
# Load & discretize REAL-LIFE (use body angle range)
